...
*   **N1: 纯抗革兰氏阳性菌 (AGP) 数据：** 800条肽段序列，均有对应的AlphaFold预测结构。
    *   **用途：** 强区分性负样本。
*   **N2: 大型AMP数据库：** 包含约3600余条序列（已包含P1, P2, N1）。目前训练出来一个准确率超过90%的AGN分类器。
...
*   **混合精度训练 (Mixed Precision)：** 启用 (如PyTorch `torch.cuda.amp`) 以加速并减少显存。
...
*   **预测AGN活性：** 使用外部的、独立的AGN活性预测器（如果可用，包括自己的分类器）评估生成序列的潜在活性。
